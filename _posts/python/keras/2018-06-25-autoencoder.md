...
tags: python python-lib machine-learning keras auto-encoder encoder decoder dimenaionality-reduction mnist matplotlib numpy 
...
- 이제 컨볼루션으로 만들어봅니다. 이제 사실 그놈이 그놈인데, convolutional layer의 경우는 encoding, decoding 시에 새로운 레이어가 하나 들어가게 되거든요. 그래서 집어넣었습니다. 
    - `Upsampling2D`가 그 아이인데, `Maxpooling2D`가 사이즈가 큰 놈을 작게 줄여준다면(downsampling), 이 아이는 작은 놈을 크게 늘려주는 형태입니다. 
...
- autoencoder fitting. 이제 시간이 오래 걸리기 시작합니다. 
...
```
Epoch 1/10
55000/55000 [==============================] - 121s - loss: 0.1735   
Epoch 2/10
55000/55000 [==============================] - 81s - loss: 0.1677    
Epoch 3/10
55000/55000 [==============================] - 81s - loss: 0.1628    
Epoch 4/10
55000/55000 [==============================] - 81s - loss: 0.1585    
Epoch 5/10
55000/55000 [==============================] - 80s - loss: 0.1547    
Epoch 6/10
55000/55000 [==============================] - 81s - loss: 0.1516    
Epoch 7/10
55000/55000 [==============================] - 81s - loss: 0.1488    
Epoch 8/10
55000/55000 [==============================] - 80s - loss: 0.1461    
Epoch 9/10
55000/55000 [==============================] - 81s - loss: 0.1437    
Epoch 10/10
55000/55000 [==============================] - 81s - loss: 0.1414    
Out[226]:
<keras.callbacks.History at 0xd1f0bf0f0>
```


```python
ks = [8, 12, 3]
f, axes = plt.subplots(3, 2, figsize=(9, 9))
for i, k in enumerate(ks):
    axes[i][0].imshow(x_train_img[[k], :].reshape(28, 28), cmap=plt.cm.gray)
    axes[i][0].set_title('original img')
    axes[i][0].axis('off')
    axes[i][1].imshow(decoder_convolution.predict(encoder_convolution.predict(x_train_img[[k], :])).reshape(28,28), 
               cmap=plt.cm.gray)
    axes[i][1].set_title('decoded img')
    axes[i][1].axis('off')
plt.savefig('../../assets/images/markdown_img/180625_autoencoder_convolutional.svg')
plt.show()
```

![](/assets/images/markdown_img/180625_autoencoder_convolutional.svg)
...
- 가장 큰 수확이라면, 차원 축소를 위해서 autoencoder/decoder를 사용할 수 있다, 는 걸 알았다는 것이겠죠. 
...
plt.show()
########################################
### auto encoder convolutional complete 
########################################
```